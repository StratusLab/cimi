...
(ns eu.stratuslab.cimi.resources.service-configuration-lifecycle-test
  (:require
...
(deftest lifecycle
  ...
  (let [uri (-> (session (ring-app))
    ...
    (let [entries (-> (session (ring-app))
                      ...
                      (t/is-count pos?)
                      (t/entries :serviceConfigurations))]
...
(deftest bad-methods
  (let [resource-uri (str base-uri "/service.instance")]
    (doall
